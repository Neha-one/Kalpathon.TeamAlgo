        ...
        <div className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-6">
            {teamMembers.map((member, idx) => (
              <div
                key={idx}
                className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 flex flex-col items-center text-center group border border-gray-100"
              >
                {/* Avatar / Image */}
                {member.image ? (
                  <img
                    src={member.image}
                    alt={member.name}
                    className="w-20 h-20 mb-4 rounded-full object-cover border-2 border-white shadow-md"
                  />
                ) : (
